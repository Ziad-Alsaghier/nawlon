...
// Driver
        $driverFollow = driverFollow::where('user_id',auth()->user()->id)->get();
                ...
                //  Car Part Count    
                $carPart = CarPart::where('user_id', auth()->user()->id)->whereMonth('created_at',
                Carbon::now()->month)->get();
                $totalCarPart = count($carPart);
return
view('user.dashboard',compact('countNawlone','nawlones',
'countCar','countDriver',
'totalNawlon','countDriverFollow',
'countMaintanence','total',
'maintanence','countavailableCar',
'countunAvailableCar','totalRevenues','countavailableDriver','totalExpenses','totalCarPart','countBusyCar'));
